...
export function NotificacoesDropdown({ userId }: { userId: string }) {
  ...
  async function marcarTodasComoLidas() {
    ...
    const { error: reflexoesPassoError } = await supabase
      .from("reflexoes_passo")
      .update({ notificacao_id: null })
      .in("notificacao_id", notificacaoIds)

    if (reflexoesPassoError) {
      console.error("[v0] Erro ao limpar referências de reflexoes_passo:", reflexoesPassoError)
    } else {
      console.log("[v0] Referências de reflexoes_passo limpas com sucesso")
    }

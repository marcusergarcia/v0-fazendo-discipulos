...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'discipulos_user_id_key'
  ) THEN
...
INSERT INTO public.discipulos (user_id, discipulador_id, nivel_atual, xp_total, fase_atual, passo_atual, aprovado_discipulador, data_aprovacao_discipulador, status)
SELECT 
  id, 
  ...
  'ativo' -- Status ativo para master
FROM auth.users 
...
SET 
  ...
  status = 'ativo'; -- Garante status ativo para master

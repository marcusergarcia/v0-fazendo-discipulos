...
BEGIN
  ...
  SELECT EXISTS(
    SELECT 1 FROM public.profiles WHERE user_id = v_apostolos_user_id
  ...
  ELSE
    RAISE NOTICE 'Perfil de 12 Apóstolos já existe.';
  END IF;
  ...
  ELSE
    RAISE NOTICE 'Registro de discípulo de 12 Apóstolos já existe.';
  END IF;

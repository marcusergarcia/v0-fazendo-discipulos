...
export default async function LeituraBiblicaPage({
  ...
  try {
    console.log("[v0] 🔷 PÁGINA LEITURA BÍBLICA INICIADA")

    const supabase = await createClient()
    console.log("[v0] ✅ Supabase client criado")
    ...
    if (authError || !user) {
      console.log("[v0] ❌ Usuário não autenticado, redirecionando")
      redirect("/auth/login")
    }
    ...
    console.log("[v0] 📊 Discípulo:", {
      hasDiscipulo: !!discipulo,
      discipuloId: discipulo?.id,
      error: discipuloError?.message,
    })

    if (!discipulo) {
      console.log("[v0] ❌ Discípulo não encontrado, redirecionando")
      redirect("/dashboard")
    }

    console.log("[v0] 📖 Buscando plano de leitura...")
    ...
    console.log("[v0] 📊 Plano de leitura:", {
      hasPlano: !!planoLeitura,
      totalSemanas: planoLeitura?.length,
      error: planoError?.message,
    })

    if (!planoLeitura || planoLeitura.length === 0) {
      console.log("[v0] ❌ Plano de leitura não encontrado, redirecionando")
      redirect("/dashboard")
    }

    console.log("[v0] 📖 Buscando capítulos lidos...")
    ...
    console.log("[v0] 📊 Leitura data:", {
      hasData: !!leituraData,
      capitulosLidosLength: leituraData?.capitulos_lidos?.length,
    ...
    console.log("[v0] 📊 Capítulos lidos processados:", capitulosLidosArray.length)
    ...
    console.log("[v0] 📊 Progresso:", {
      semanasConcluidas: semanasConcluidas.size,
      semanasEmProgresso: semanasEmProgresso.size,
    })
    ...
    console.log("[v0] 📊 Search params:", params)
    ...
    console.log("[v0] 📊 Semana selecionada:", {
      semanaAtual,
      semanaSelecionada,
      hasLeituraAtual: !!leituraAtual,
    })

    const temSemanaAnterior = semanaSelecionada > 1
    const temProximaSemana = semanaSelecionada < 52

    console.log("[v0] ✅ PÁGINA RENDERIZANDO COMPONENTES")
    ...
    console.error("[v0] ❌ ERRO FATAL NA PÁGINA:", error)
    console.error("[v0] ❌ Stack trace:", (error as Error).stack)
    console.error("[v0] ❌ Message:", (error as Error).message)
    console.error("[v0] ❌ Name:", (error as Error).name)

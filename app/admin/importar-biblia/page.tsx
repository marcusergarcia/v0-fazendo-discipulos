...
import { atualizarCapitulo, buscarCapitulosVazios, diagnosticarCapitulos } from './actions'
...
export default function ImportarBibliaPage() {
  ...
  const [diagnostic, setDiagnostic] = useState<{totalCapitulos: number, capitulosVazios: number, capitulosPreenchidos: number} | null>(null)
  ...
  const importarBiblia = async () => {
    ...
    try {
      ...
      for (let i = 0; i < capitulos.length; i++) {
        ...
        const livroJSON = bibliaJSON.find((l: any) => l.abbrev?.toLowerCase() === abreviacao)
        ...
        const texto = versiculos.join(' ')
        
        if (texto && texto.trim().length > 0) {
          const { data: updateData, error: updateError } = await atualizarCapitulo(cap.id, texto)
          ...
          {diagnostic && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <ul className="space-y-1 text-sm">
                <li>📊 Total de capítulos: <strong>{diagnostic.totalCapitulos}</strong></li>
                <li>📝 Capítulos vazios (sem texto): <strong>{diagnostic.capitulosVazios}</strong></li>
                <li>✅ Capítulos preenchidos: <strong>{diagnostic.capitulosPreenchidos}</strong></li>
              </ul>
              {diagnostic.totalCapitulos === 0 && (
                <p className="mt-3 text-amber-700 text-sm">
                  ⚠️ A tabela está vazia! Execute o script <code className="bg-amber-100 px-1 rounded">03-gerar-capitulos-vazios.sql</code> primeiro.
                </p>
              )}
            </div>
          )}
